...
def material_log(needed_materials):
    ...
    if st.session_state.log_state is 0:
        st.write("The material log is in Edit Mode. Choose the category of material and the name to input how many of the material you have. To view all materials of a current category, press this button:")
        ...
        with st.form('material_amount'):
            st.number_input("Please input the number of " + st.session_state[material_type] + " " + material_type + "s you have.",
...
def recipe_synthesis():
    st.html("<h4>This is the synthesis page. This has a list of every craftable item, along with their recipe. You "
            ...
            "entered the values into the Material Log. Try crafting some yourself!</h4><p>Note: Crafting recipes past "
            "Group 1 are locked until the requirements are met.")
    i = 1
    group = 1
    crafted = 0
    reset = st.button("Reset recipes")
    st.html("<h2>Group " + str(group) + ":</h2>")
    org1, org2, org3 = st.columns([1, 1, 1])

    for element in st.session_state.recipes:
        if st.session_state.recipes[element]['group'] is not group:
            group = st.session_state.recipes[element]['group']
            if group is 2 and crafted < 3:

                st.html("<h3>Sorry, Group 2 isn't unlocked yet. Craft " + str(3 - crafted) + " more items to be able "
                                                                                             "to unlock it.")
                break
            elif group is 3 and crafted < 9:
                st.html("<h3>Sorry, Group 3 isn't unlocked yet. Craft " + str(9 - crafted) + " more items to be able "
                                                                                             "to unlock it.")
                break
            elif group is 4 and crafted < 15:
                st.html("<h3>Sorry, Group 4 isn't unlocked yet. Craft " + str(15 - crafted) + " more items to be able "
                                                                                              "to unlock it.")
                break
            elif group is 5 and crafted < 21:
                st.html("<h3>Sorry, Group 5 isn't unlocked yet. Craft " + str(21 - crafted) + " more items to be able "
                                                                                              "to unlock it.")
                break
            elif group is 6 and crafted < 30:
        ...
        if i is 1:
            with org1:
                crafted += create_dropdown(element)
            i += 1
        elif i is 2:
            with org2:
                crafted += create_dropdown(element)
    ...
    if reset:
        for element in st.session_state.recipes:
            st.session_state.recipes[element]['crafted'] = False
        st.rerun()


def create_dropdown(element):
    st.write(element)
    crafted = st.session_state.recipes[element]['crafted']
    with st.popover(":white_check_mark:" if crafted else " :x:"):
        if crafted:
            ...
            for key in st.session_state.recipes[element]:
                category = 0
                name = 0
                if len(key.split(" ")) is 2:
                    if key.split(" ")[1] is 'Misc':
                        st.write(key.split(" ")[0] + ": " + str(
                            ...
                            st.session_state.recipes[element][key]))
            ...
            if to_Craft:
                for key in st.session_state.recipes[element]:
                    ...
                    if st.session_state.have_materials[category][name] < st.session_state.recipes[element][key]:
                        st.error("Oh no! You're missing " + str(
                            st.session_state.have_materials[category][name] - st.session_state.recipes[element][
                                key] * -1) + " " + ((name + " " + category) if category is not 'Misc' else name) + (
                                     "s. " if st.session_state.have_materials[category][name] -
                                              st.session_state.recipes[element][key] * -1 != 1 else ". "))
                        return 0
                st.session_state.recipes[element]['crafted'] = True
                for key in st.session_state.recipes[element]:
                    if len(key.split(" ")) is 2:
                        category = key.split(" ")[1]
                        name = key.split(" ")[0]
                        ...
                        category = key.split(" ")[2]
                        name = key.split(" ")[0] + " " + key.split(" ")[1]
                    st.session_state.have_materials[category][name] -= st.session_state.recipes[element][key]
                st.rerun()
